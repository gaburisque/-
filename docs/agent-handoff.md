...
最終更新日: 2026-05-18（授業記録入力画面リニューアル・コース管理追加・UI整理）
...
- `/lesson-records`: 授業記録一覧（コース・曜日・年フィルター、CSV出力）
- `/lesson-records/new`: **授業記録入力（全面リニューアル済み）**。曜日タブ・日付選択・生徒リスト（記録済/下書き/未記録バッジ）・前回記録折りたたみ・構造化フォーム・下書き保存
...
- **授業記録入力画面（/lesson-records/new）全面リニューアル（2026-05-18）**
  ...
  - 前回記録の曜日を優先して生徒を振り分け。前回なければ enrollments.weekday を使用
...
- 授業記録一覧の「仮データ追加」ボタン：開発環境のみ表示（`NODE_ENV === "development"`）。仮生徒12名・受講登録・直近8週分の授業記録を upsert 投入
- 出欠管理（日付選択 + 一括保存）
- 教材・書類アップロード
- 週間スケジュールと担当講師割り当て
- 生徒情報CSV出力
